...
export class Downloader extends DownloaderBase {
    ...
    public createDownload(options: DownloadOptions): string {
        ...
        if (options.path && options.fileName) {
            ...
            filePath = path.join(knownFolders.temp().path, options.fileName);
        } else if (options.path && !options.fileName) {
            filePath = path.join(options.path, `${generateId()}`);
        } else {
            filePath = path.join(knownFolders.temp().path, `${generateId()}`);
        }
                    ...
                    if (state === NSURLSessionTaskState.Running) {
                        ...
                        if (owner.downloadsData.has(id)) {
                            ...
                            if (data) {
                                if (
                                    data.status &&
                                    data.status !== StatusCode.DOWNLOADING
                                ) {
                            ...
                            if (
                                ...
                                const updateBytes =
                                    currentBytes - lastBytesWritten;
                                speed = Math.floor(
                                    Math.round(updateBytes / intervalTime)
                                );
